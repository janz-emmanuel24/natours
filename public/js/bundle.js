    ...
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            res = _context2.sent;
            if (res.data.status = 'success') location.reload(true); // this will force a reload from the server not the browser
...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false // center: [-118.121010, 34.112045],
    // zoom: 8,
    // interactive: false

  });
  var bounds = new mapboxgl.LngLatBounds(); //this is the area that will be displayed on the map
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extend the map boundaries to include current locations
...
var stripe = Stripe('pk_test_51H0OgnGaieo91b260PlaNR9gaYsSajPSX5bRDpZgmyxvjE879izQDz6BddxZC3KPMzZDbyZcnlYPDdamNDwLYKMo00Fl0OC849');
...
var bookBtn = document.getElementById('book-tour'); // Check if the element exists and then delegate it 
